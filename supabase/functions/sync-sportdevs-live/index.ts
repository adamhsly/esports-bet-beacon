...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🔴 Starting SportDevs live matches sync...');
    ...
    for (const esportType of ESPORT_TYPES) {
      try {
        ...
        const response = await fetch(`https://api.sportdevs.com/esports/${gameId}/matches/live`, {
          headers: {
            'x-api-key': apiKey,
        ...
        if (!response.ok) {
          console.error(`❌ Error fetching ${esportType} live matches: ${response.status}`);
          continue;
        }
