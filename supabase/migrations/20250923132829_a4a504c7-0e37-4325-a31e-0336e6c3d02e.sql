-- Update get_faceit_teams_prev_window_stats to use match_date for better performance
...
-- Use match_date index for much faster lookups
WITH match_window AS (
  SELECT *
  FROM public.faceit_matches
  WHERE match_date >= start_date
...
-- Update get_all_faceit_teams to optionally filter by date range for better performance
CREATE OR REPLACE FUNCTION public.get_all_faceit_teams(start_date date DEFAULT NULL, end_date date DEFAULT NULL)
RETURNS TABLE(
  team_id text,
  team_name text,
  logo_url text,
...
WITH t AS (
  SELECT lower(trim(teams->'faction1'->>'name')) AS team_key,
         ...
         game
  FROM public.faceit_matches
  WHERE (start_date IS NULL OR match_date >= start_date)
