-- Fix get_platform_daily_stats function - correct column names
DROP FUNCTION IF EXISTS get_platform_daily_stats(timestamptz, timestamptz);

CREATE OR REPLACE FUNCTION get_platform_daily_stats(p_start timestamptz, p_end timestamptz)
RETURNS TABLE (
  stat_date DATE,
  new_users BIGINT,
...
BEGIN
  ...
  daily_real_revenue AS (
    SELECT re.created_at::date AS d, 
           COALESCE(SUM(CASE WHEN re.promo_used = false THEN re.amount_paid ELSE 0 END), 0) AS amt
    FROM round_entries re
    INNER JOIN profiles pf ON pf.id = re.user_id
    WHERE re.created_at >= p_start AND re.created_at <= p_end
      AND re.status = 'completed'
  ...
  daily_bonus_used AS (
    SELECT re.created_at::date AS d, 
           COALESCE(SUM(CASE WHEN re.promo_used = true THEN re.amount_paid ELSE 0 END), 0) AS amt
    FROM round_entries re
    INNER JOIN profiles pf ON pf.id = re.user_id
    WHERE re.created_at >= p_start AND re.created_at <= p_end
      AND re.status = 'completed'
