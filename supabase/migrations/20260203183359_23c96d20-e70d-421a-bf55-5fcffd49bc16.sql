-- Fix Tier 1 welcome bonus to grant only ONE free entry (250 pence)
-- and prevent re-claiming Tier 1 after a promo-covered entry was already used.
...
DECLARE
  ...
  v_has_used_promo_entry BOOLEAN;
BEGIN
  ...
  -- If user has ever completed a promo-covered entry, Tier 1 must never be claimable again
  SELECT EXISTS (
    SELECT 1
    FROM round_entries
  ...
  ELSE
    ...
    IF v_has_used_promo_entry THEN
      RETURN json_build_object(
        'success', false,
        'error', 'Tier 1 welcome bonus already used.',
        'tier', 1
      );
    END IF;

    v_tier := 1;
    v_reward_pence := 250; -- £2.50 (exactly one paid round entry)
  END IF;
