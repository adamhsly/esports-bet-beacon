-- Drop and recreate get_team_stats_optimized function with correct column names
...
BEGIN
  ...
  SELECT
    COUNT(*) FILTER (
      WHERE (
        (m.teams->0->>'id' = p_team_id AND m.winner_id = p_team_id)
        OR (m.teams->1->>'id' = p_team_id AND m.winner_id = p_team_id)
      )
    )::integer,
    COUNT(*)::integer,
    COALESCE(
  ...
  WHERE m.status = 'finished'
    ...
    AND (
      m.teams->0->>'id' = p_team_id
      OR m.teams->1->>'id' = p_team_id
    );
