...
import { Target, TrendingUp, Loader2, Trophy, Calendar } from 'lucide-react';
import { supabase } from '@/integrations/supabase/client';

interface AmateurTeamStatsModalProps {
  isOpen: boolean;
  onClose: () => void;
  team: {
    id: string;
...
interface FaceitTeamForm {
  ...
  wins?: number;
  losses?: number;
  recent_win_rate_30d?: number;
}

export const AmateurTeamStatsModal: React.FC<AmateurTeamStatsModalProps> = ({
  isOpen,
  onClose,
  team
}) => {
  const [stats, setStats] = useState<FaceitTeamForm | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      setLoading(true);
      try {
        // Use faceit_team_form function to get amateur team stats
        const { data, error } = await supabase.rpc('faceit_team_form', {
          team_name: team.name,
          game_filter: team.esport_type || null
        });

        if (error) {
          console.error('Error fetching amateur team stats:', error);
          setStats(null);
        } else {
          setStats(data as FaceitTeamForm);
        }
      } catch (error) {
        console.error('Error fetching amateur team stats:', error);
        setStats(null);
  ...
  }, [isOpen, team?.name, team?.esport_type]);
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-0.5 sm:gap-1">
                  {(stats.recent_form || 'N/A').split('').map((char, index) => (
                    <span
                      key={index}
                      className={`
              ...
              {stats.recent_win_rate_30d !== undefined && (
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                  ...
                  <Badge className="bg-blue-500/20 text-blue-400 border-blue-400/30">
                    {stats.recent_win_rate_30d}%
                  </Badge>
                </div>
              )}

              {stats.tournament_wins !== undefined && stats.tournament_wins > 0 && (
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
              ...
              <div className="text-xs text-gray-400 mt-2">
                Record: {stats.wins || 0}W - {stats.losses || 0}L ({stats.total_matches || 0} total)
              </div>
            </div>
          ) : (
            <div className="space-y-3">
