...
export const TeamFiltersOverlay: React.FC<TeamFiltersOverlayProps> = ({
  ...
  const proWinRates = proTeams.map(t => t.recent_win_rate ?? 0);
  const amateurMatchVolumes = amateurTeams.map(t => t.matches_prev_window ?? 0);
  const amateurCredits = amateurTeams.map(t => t.price ?? 0);
  const amateurAbandonRates = amateurTeams.map(t => t.abandon_rate ?? 0);
  const proRanges = {
    matches: {
      min: Math.min(...proMatchVolumes),
      max: Math.max(...proMatchVolumes)
    },
    credits: {
      min: Math.min(...proCredits),
      max: Math.max(...proCredits)
    },
    winRate: {
      min: Math.min(...proWinRates),
      max: Math.max(...proWinRates)
    }
  };
  const amateurRanges = {
    matches: {
      min: Math.min(...amateurMatchVolumes),
      max: Math.max(...amateurMatchVolumes)
    },
    credits: {
      min: Math.min(...amateurCredits),
      max: Math.max(...amateurCredits)
    },
    abandonRate: {
      min: Math.min(...amateurAbandonRates),
      max: Math.max(...amateurAbandonRates)
    }
  };
  useEffect(() => {
        ...
        <div className="mt-6 space-y-6">
          {activeTab === 'pro' ? <>
              ...
              <div className="space-y-3">
                ...
                <Slider value={filters.pro.matches} onValueChange={value => setFilters(prev => ({
              ...prev,
              pro: {
                ...prev.pro,
                matches: value
              }
            }))} min={proRanges.matches.min} max={proRanges.matches.max} step={1} className="w-full" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <Slider value={filters.pro.credits} onValueChange={value => setFilters(prev => ({
              ...prev,
              pro: {
                ...prev.pro,
                credits: value
              }
            }))} min={proRanges.credits.min} max={proRanges.credits.max} step={1} className="w-full" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <Slider value={filters.pro.winRate} onValueChange={value => setFilters(prev => ({
              ...prev,
              pro: {
                ...prev.pro,
                winRate: value
              }
            }))} min={proRanges.winRate.min} max={proRanges.winRate.max} step={1} className="w-full" />
              </div>
            </> : <>
              {/* Matches Filter */}
              <div className="space-y-3">
                ...
                <Slider value={filters.amateur.matches} onValueChange={value => setFilters(prev => ({
              ...prev,
              amateur: {
                ...prev.amateur,
                matches: value
              }
            }))} min={amateurRanges.matches.min} max={amateurRanges.matches.max} step={1} className="w-full" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <Slider value={filters.amateur.credits} onValueChange={value => setFilters(prev => ({
              ...prev,
              amateur: {
                ...prev.amateur,
                credits: value
