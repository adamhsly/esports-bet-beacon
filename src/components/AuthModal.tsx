...
import { Mail, Lock, User, AlertCircle, CheckCircle2, X, CalendarIcon } from 'lucide-react';
...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess, defaultTab }) => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const isUnder18 = age < 18 || (age === 18 && (monthDiff < 0 || (monthDiff === 0 && today.getDate() < dateOfBirth.getDate())));
    
    if (isUnder18) {
      const errorMessage = "You must be at least 18 years old to create an account.";
      setSignUpError(errorMessage);
      toast({
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className={cn(
                        "w-full justify-start text-left font-normal bg-theme-gray-dark border-theme-gray-light text-white hover:bg-theme-gray-dark/80",
                  ...
                  <PopoverContent className="w-auto p-0 bg-theme-gray-dark border-theme-gray-light" align="start">
                    <Calendar
                      mode="single"
