...
const TEAM_LOGO_PLACEHOLDER = '/placeholder.svg';
...
function isValidUrl(url?: string | null) {
  if (!url) return false;
  if (url === TEAM_LOGO_PLACEHOLDER) return false;
...
export function getEnhancedTeamLogoUrl(team: {
  ...
  return mapped ?? TEAM_LOGO_PLACEHOLDER;
}

/**
 * Guard: prevent accidental imports of a conflicting name.
 */
